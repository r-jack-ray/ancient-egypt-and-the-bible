...
- Python: do not assume `python` is on `PATH`. Prefer `C:\Toolbox\Python313\python.exe` for repository helpers and skill validation; it includes PyYAML for `quick_validate.py`. If that installation is unavailable, use the Python executable reported by `codex_app.load_workspace_dependencies`, typically `C:\Users\JR\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe`. If a Python helper still fails because a package is missing, report the missing module and fall back to a manual check when practical.
