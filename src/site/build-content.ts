...
import { parseStreamIndex } from "../archive.js";
...
const generatedPageMarker =
  "<!-- Generated by npm run build:site-content from SOURCE. Do not edit this mirror by hand. -->";
...
export async function buildHugoSiteContent(
  ...
  const liveStreamListPath = join(repoRoot, "src/live-stream-list.md");
  ...
  await requireFile(liveStreamListPath);
  ...
  const archiveEpisodes = parseStreamIndex(await readFile(liveStreamListPath, "utf8"));
  if (archiveEpisodes.length === 0) {
    throw new Error(`No episodes parsed from ${liveStreamListPath}.`);
  }
  ...
  for (const fileName of questionFileNames) {
    ...
    if (numberedMatch?.[1] !== undefined) {
      ...
      if (episode === undefined) {
        throw new Error(
          `${sourcePath} starts with episode number ${number}, but no matching entry exists in src/live-stream-list.md.`,
        );
      }
      pageMetadata = {
        number,
    ...
    const frontMatter = [
      ...
      "---",
      "",
      generatedPageMarker.replace("SOURCE", sourceRelativePath),
      "",
    ];
...
function questionRowsFromMarkdown(
  ...
  for (let index = tableStart + 2; index < lines.length; index += 1) {
    ...
    rows.push({
      ...
      search_text: [pageMetadata.number, pageMetadata.title, question, shortAnswer, expandedAnswer]
        .map((value) => value ?? "")
        .join(" ")
        .toLowerCase(),
    });
