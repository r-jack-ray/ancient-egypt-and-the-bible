...
Status: implemented on 2026-08-01 and reviewed on 2026-08-02. The two stages remain separate and independently rerunnable. Inventory continues to reuse discovery metadata under guarded apply, with added no-addition and multiple-addition fixtures. The caption batch paces transcript attempts across episode boundaries, always skips known-unavailable records, applies limits consistently during dry runs, checkpoints partial failures, scans the remaining state without further requests after blocking evidence, and prints a deterministic handoff listing new TXT paths and all deferred, failed, or pending records. Review corrected recorded failures being skipped indefinitely by the ordinary weekly command, removed the resulting redundant retry option, clears stale failure status for stored or known-unavailable records, and ensures a primary-provider CAPTCHA trips the circuit breaker without a fallback request. On 2026-08-08, pacing moved from each provider-internal HTTP request to the boundary between transcript attempts so one selected transcript is fetched and written without artificial minute-long internal waits. The unused single-video command and its self-referential documentation were removed; the README `latest` invocation named by the earlier analysis was already absent in the implementation worktree.
...
The current worktree now exposes `fetch:livestreams` as the only public Google inventory command. As of 2026-08-08, its default registers the newest numbered livestream and pins the resolved source when needed; `--review-only` is an explicit diagnostic mode. Do not restore a second `fetch:livestreams:latest` alias. Keep README examples on the base command.
...
2. **Caption scrape to canonical TXT**
   ...
   - Preserve conservative spacing between transcript attempts and resumable/checkpointed behavior so a partial run can be rerun safely; do not delay the internal requests for one transcript.
...
- Caption-scraper fixtures cover multiple missing TXT files, existing valid TXT no-ops, the two known-unavailable skips, captions not yet ready, partial failure and rerun, direct TXT output, inter-transcript spacing, and no delay within one transcript fetch.
...
11. Completed Phase 4 with concise explicit diagnostic inventory deltas, report-free ordinary inventory updates, failure-focused question-table diagnostics with CI-visible errors, retirement of the unconsumed question-revision report implementation, the renamed transcript status command, and documented report ownership and cleanup. The inventory default changed on 2026-08-08 to register the newest numbered livestream.
