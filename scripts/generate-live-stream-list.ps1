...
.VERSION
v010 - Uses only the /streams Live tab by default and writes entries in Live-tab chronological order by default.
...
Default order follows the /streams Live tab order, newest-to-oldest, with non-numbered streams interleaved.
...
# DisplayOrder tracks first discovery from the /streams tab so non-numbered streams can stay
...
function Add-VideoItemsFromJson {
    ...
    foreach ($object in (Get-AllObjectsFromJson -Root $JsonObject)) {
        $videoId = Get-FirstVideoIdFromNode -Node $object
        if ([string]::IsNullOrWhiteSpace($videoId)) { continue }

        $title = Get-FirstTitleFromNode -Node $object -RequireNumberedLiveStream $RequireNumberedLiveStream
...
function Read-YoutubeBrowseSource {
    ...
    # Parse structured JSON first because its object/array traversal preserves the Live-tab page order
    # better than regex fallback scans. Raw-text extraction still runs afterward to recover edge cases.
    ...
    foreach ($token in (Get-ContinuationTokensFromJson -JsonObject $pageData.InitialData)) { $initialTokens.Add($token) }
    foreach ($token in (Get-ContinuationTokensFromText -Text $html)) { $initialTokens.Add($token) }
    ...
    while ($pendingContinuations.Count -gt 0) {
        ...
        try {
            ...
            foreach ($nextToken in (Get-ContinuationTokensFromJson -JsonObject $response)) {
                if ($seenContinuations.Add($nextToken)) { $pendingContinuations.Enqueue($nextToken) }
            }
        }
        catch {
            Write-Warning "[$SourceName] Could not parse continuation response as JSON on page $continuationPagesRead. Raw-text extraction will still run."
        }
...
if ($items.Count -eq 0) {
    throw "No matching livestream entries were found. Re-run with -WriteDebugDump to save raw YouTube responses under: $debugDirectory"
}
...
elseif ($OldestFirst) {
    Write-Host 'Output order: /streams Live-tab order reversed, oldest discovered stream first.'
}
else {
    Write-Host 'Output order: /streams Live-tab order, newest discovered stream first.'
}
